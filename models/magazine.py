class Magazine:
    ...
    def articles(self, cursor):
        # getting a list of articles in the magazine
        cursor.execute("SELECT * FROM articles WHERE magazine_id = ?", (self._id,))
        articles_data = cursor.fetchall()
        return articles_data

    def contributors(self, cursor):
        # getting a list of authors who have contributed to the magazine
        cursor.execute("""
            SELECT authors.*
            FROM authors
    ...
    def article_titles(self, cursor):
        # getting a list of article titles associated with the magazine
        cursor.execute("SELECT title FROM articles WHERE magazine_id = ?", (self._id,))
        titles = [row[0] for row in cursor.fetchall()]
        return titles

    def contributing_authors(self, cursor):
        # getting a list of authors who have contributed more than two articles to the magazine
        cursor.execute("""
